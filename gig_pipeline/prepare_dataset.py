"""
Merge candidate-beta JSONL with raw HotpotQA / MuSiQue / 2Wiki context and write
...
Usage:
    ...
    python prepare_dataset.py \
        --dataset 2wiki \
        --final  output/2wiki/2wiki_final.jsonl \
        --output output/2wiki/2wiki_candidate_train.jsonl
"""
...
TWOWIKI_RAW = [
    f"{DATA_ROOT}/2WikiMultiHopQA/data_ids_april7/train.json",
    f"{DATA_ROOT}/2WikiMultiHopQA/data_ids_april7/dev.json",
]
...
def load_2wiki_originals() -> dict:
    out = {}
    for path in TWOWIKI_RAW:
        with open(path) as f:
            for sample in json.load(f):
                if sample.get("type") == "bridge_comparison":
                    continue
                out[sample["_id"]] = sample
    return out
...
CONFIGS = {
    ...
    "2wiki":   (load_2wiki_originals,   hotpot_context),
}
