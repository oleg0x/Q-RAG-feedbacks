...
class RetrievalHotPotQA(Dataset):
    def __init__(self,
        ...
        if split not in ['train', 'eval', 'all', 'fullwiki_eval', 'train-clean']:
    ...
    def _load_data(self, path):
        ...
        raw_tasks = []
        if self.split == 'train-clean':
            if os.path.isfile(path):
                file_path = path
            else:
                filenames = [
                    self.clean_filename,
                    "hotpot_candidate_train_q0_s1.jsonl",
                    "hotpot_train_candidates_q0_s1.jsonl",
                ]
                candidates = [
                    os.path.join(path, name) for name in filenames if name
                ]
                file_path = next((name for name in candidates if os.path.exists(name)), candidates[0])
            with open(file_path, 'r', encoding='utf-8') as json_file:
                self.tasks = [json.loads(line) for line in json_file if line.strip()]
