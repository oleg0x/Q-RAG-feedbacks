...
def _run_coroutine_sync_llm(factory):
    try:
...
class LLMGenerator:
    ...
    def __init__(
        ...
        super().__init__()
        if prepare_messages_func is not None and not callable(prepare_messages_func):
            raise TypeError(
                f"LLMGenerator.prepare_messages_func must be callable, got "
                f"{type(prepare_messages_func).__name__}: {prepare_messages_func!r}. "
                "If Hydra interpolation turned this into a string, check overrides: "
                "use dotted keys such as feedback._formatter._target_=..., not "
                "feedback.prompt_formatter=prompt_formatter._target_=..."
            )
        ...
        async with self._session_lock:
            if self._session is None or self._session.closed:
                ...
                self._session = aiohttp.ClientSession(
                    connector=connector,
                    timeout=aiohttp.ClientTimeout(total=15.0, connect=2.0),
                    headers=headers
                )
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                payload = {
                    ...
                    **{
                        k: v
                        for k, v in self.answer_sampling_params.items()
                        if v is not None
                    },
                }
    ...
    # Compatibility API for InfoGainFeedback (expects BasicVllmClient-like methods).
    def _build_chat_messages_compat(
        self,
        query: Optional[str] = None,
        examples: Optional[List[Tuple[str, str]]] = None,
        passages: Optional[List[str]] = None,
    ) -> list[dict]:
        if examples is not None and query is not None:
        ...
        if passages is None:
            passages = []
        return self._build_answer_messages(query, passages)

    @staticmethod
    def _choice_cumulative_logprob(choice: dict) -> Optional[float]:
        ...
        total = 0.0
        for tok in content:
            total += float(tok.get("logprob", 0.0))
        return total
    ...
    async def _chat_completion_api(
        ...
        *,
        ...
        t = self.answer_sampling_params.get("temperature", 0.0) if temperature is None else temperature
        p = self.answer_sampling_params.get("top_p", 0.95) if top_p is None else top_p
        max_tokens = self.answer_sampling_params.get("max_tokens", 32)
        stop = self.answer_sampling_params.get("stop", None)
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        raise RuntimeError(f"chat/completions HTTP {response.status}: {body[:400]}")
                    result = await response.json()
        else:
            resp = await asyncio.wait_for(
                self.answer_api_client.chat.completions.create(
        ...
        for choice in (result.get("choices") or []):
            ...
            cumulative_lp = self._choice_cumulative_logprob(choice) if logprobs else None
            out.append((text, cumulative_lp))
        if not out:
            out = [("", None)]
        return out
    ...
    def chat_completion(
        ...
        *,
        ...
        messages = self._build_chat_messages_compat(query, examples, passages)

        async def _run():
            res = await self._chat_completion_api(messages, n=1, logprobs=logprobs)
            return res[0]

        if self.answer_use_api:
            return _run_coroutine_sync_llm(lambda: _run())
        text = self.generate_answer(query or "", passages or [])
        return text, None

    def chat_completion_n(
        self,
        query: str,
        n: int,
        examples: Optional[List[Tuple[str, str]]] = None,
        ...
        *,
        ...
        if self.answer_use_api:
            return list(_run_coroutine_sync_llm(lambda: _run()))
        # local-vLLM fallback: sample independently n times.
        t = self.answer_sampling_params.get("temperature", 0.0) if temperature is None else temperature
        p = self.answer_sampling_params.get("top_p", 0.95) if top_p is None else top_p
        sp = {**self.answer_sampling_params, "temperature": t, "top_p": p}
        prompts = self._prepare_prompts([query] * n, [passages or []] * n)
        outputs = self.answer_model.generate(prompts=prompts, sampling_params=SamplingParams(**sp))
        return [(o.outputs[0].text.strip(), None) for o in outputs]

    # ── logprobs-aware generation (for InfoGainFeedback Eq. 13) ──────

    async def _fetch_local_api_logprobs(self, messages) -> tuple:
        """Like _fetch_local_api but returns (text, cumulative_logprob)."""
        if self._shutdown:
            return ("", 0.0)
        try:
            connector = aiohttp.TCPConnector(limit=1, force_close=True)
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        content = lp_data.get('content') or []
                        for tok in content:
                            lp += tok.get('logprob', 0.0)
                        return (text, lp)
                    else:
                        logger.warning(f"API logprobs error: {response.status}")
                        return ("", 0.0)

        except asyncio.TimeoutError:
            logger.warning("Local API logprobs timeout")
            return ("", 0.0)
        except Exception as e:
            logger.warning(f"Local API logprobs error: {e}")
            return ("", 0.0)

    async def _fetch_remote_api_logprobs(self, messages) -> tuple:
        """Like _fetch_remote_api but returns (text, cumulative_logprob)."""
        try:
            resp = await asyncio.wait_for(
                self.answer_api_client.chat.completions.create(
                    model=self.answer_model_name,
            ...
            if resp.choices[0].logprobs and resp.choices[0].logprobs.content:
                lp = sum(t.logprob for t in resp.choices[0].logprobs.content)
            return (text, lp)
        except Exception:
            return ("", 0.0)

    def generate_answers_with_logprobs(
        self, questions: list[str], pred_chunks_list: list[list[str]]
    ) -> list[tuple[str, float]]:
        """
        Like generate_answers but also returns cumulative token logprob
        per answer, needed for likelihood-weighted belief estimation (Eq. 13).
        """
        prompts = self._prepare_prompts(questions, pred_chunks_list)
        ...
        if self.answer_use_api:
            async def _process():
                if self.is_local_api:
                    tasks = [self._fetch_local_api_logprobs(m) for m in prompts]
                else:
                    tasks = [self._fetch_remote_api_logprobs(m) for m in prompts]
                return await asyncio.gather(*tasks)

            return list(_run_coroutine_sync_llm(lambda: _process()))
        else:
            sp = {**self.answer_sampling_params, "logprobs": 1}
            outputs = self.answer_model.generate(
                prompts=prompts,
            ...
            return [
                (
                    ...
                    out.outputs[0].cumulative_logprob if out.outputs[0].cumulative_logprob is not None else 0.0,
                )
                for out in outputs
            ]
