...
class BertPredictor(nn.Module):
    ...
    def __init__(self, bert: RobertaModel, num_hidden_layers, tokenizer, model_dim, output_size, n_output) -> None:
        ...
        # Clone weights from the Hydra-instantiated checkpoint, then drop `bert`
        # so each from_pretrained peak does not linger until GC.
        emb_state = {k: v.clone() for k, v in bert.embeddings.state_dict().items()}
        layer_states = [
            {k: v.clone() for k, v in bert.encoder.layer[i].state_dict().items()}
            for i in range(config.num_hidden_layers)
        ]
        del bert

        self.model = AutoModel.from_config(config)
        self.model.embeddings.load_state_dict(emb_state)
        for i in range(config.num_hidden_layers):
            self.model.encoder.layer[i].load_state_dict(layer_states[i])
