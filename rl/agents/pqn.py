...
class PQN(object):
    ...
    def __init__(self, config: DictConfig):
        ...
        self.num_runs = config.get('pqn', {}).get('hyperparams', {}).get('num_runs', 1)
        self.alpha = config.pqn.hyperparams.alpha
        self.alpha_start = self.alpha
        #print(f"self.num_runs={self.num_runs}, self.alpha={self.alpha}")
    ...
    def select_action_batch(self, state: TextMemory, a_embeds: Tensor,  a_embeds_target: Tensor, evaluate=False, random=False):
        ...
        a = torch.tensor(self.alpha) if self.num_runs == 1 else torch.tensor(self.alpha_start)
        action, q_values, q_values_target = policy_apply(self.policy, self.v_net_target, torch_state, a_embeds,  a_embeds_target, a, evaluate)
        if random:
