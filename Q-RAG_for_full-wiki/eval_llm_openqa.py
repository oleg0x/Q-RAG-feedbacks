...
parser.add_argument('--think', action="store_true", help='enable_thinking for Qwen3 models.')
...
for data in tqdm(dataset, desc="Preparing prompts"):
    ...
    messages = [
        {
            "role": "system",
            "content": prompts.sys_qa if args.q_icl_prompt else qa_instruction_prompt,
        },
...
if args.llm_judge:
    ...
    judge_outputs = llm.generate(
        judge_prompts, SamplingParams(max_tokens=32, temperature=0.0)
    )
    for result, output in zip(results, judge_outputs):
        judgment = output.outputs[0].text.upper()
        score = float("CORRECT" in judgment and "INCORRECT" not in judgment)
...
avg_judge = (
    sum(all_judge_scores) / len(all_judge_scores) if all_judge_scores else None
)
...
if avg_judge is not None:
    print(f"Mean LLM-Judge Score: {avg_judge:.4f}")
print("=" * 50)
